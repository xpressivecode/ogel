...
ogel.preview = function(options, done){
    this.init(options);
    ...
    this.files.forEach(function(file){
        ...
        var model = self.model = self.parseModel(html);
...
ogel.build = function(options){
    this.preview(options, function(args){
        grunt.file.write(args.file, args.html);
    });
};
...
ogel.init = function(options){
    ...
    if(!fs.existsSync(options.src))throw new Error("The src path specified doesn't exist: " + options.src + ". Perhaps path.resolve() might help");
    if(!fs.existsSync(options.templateDir))throw new Error("The templateDir path specified doesn't exist: " + options.templateDir + ". Perhaps path.resolve() might help");
